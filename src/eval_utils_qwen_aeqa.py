...
def get_step_info(step, verbose=False):
    ...
    for i, rgb_id in enumerate(step["snapshot_imgs"].keys()):
        ...
        snapshot_imgs.append(tensor2pil(snapshot_img))
    ...
    snapshot_names = list(step["snapshot_imgs"].keys())
    filtered_names = [snapshot_names[keep] for keep in keep_index]
    logging.info(f"Prefiltered snapshot: {filtered_names}")
...
def format_explore_prompt(
    ...
    text += "Output only the choice line followed immediately by the reason (or answer) on the next line, with no prefix, header, or extra lines before the choice. "
...
def explore_step(step, cfg, verbose=False):
    ...
    for _ in range(retry_bound):
        full_response = call_qwen_local(sys_prompt, content)
